...
void rtinit3() 
{
   ...
   printf("Time t=%.2f        NODE 3:  Initial Distance Table\n",get_clock());
   printdt_3(&dt[id]);
   printf("\n");
   ...
   for(k = 0; k < NODES; k++){
      ...
      printf("Time t=%.2f        NODE 3:  Sending cost update to Node %d:",get_clock(),k);
...
void rtupdate3 (struct rtpkt *rcvdpkt)
{
   ...
   printf("Time t=%.2f        NODE 3:  Received cost update from Node %d:",get_clock(),sourceid);
   ...
   // 向每个邻居节点发送更新数据包
   if(change == 1){
      printf("Time t=%.2f        NODE 3:  New minimum costs.\n");
      int k,t;
      for(k = 0; k < NODES; k++){
         if(k == id || linkcost[id][k] == INF) continue;
         struct rtpkt packet;
   ...
   else printf("Time t=%.2f        NODE 3:  No change in minimum costs.\n\n");
}
...
void printdt_3 (struct distance_table *t)
{
   ...
   printf("\t\tdest 1|  %3d   %3d   %3d\n", t->costs[1][0], t->costs[1][2], t->costs[1][4]);
