...
void rtinit0() 
{
   ...
   printf("Time t=%.2f        NODE 0:  Initial Distance Table\n",get_clock());
   printdt_0(&dt[id]);
   ...
   for(k = 0; k < NODES; k++){
      ...
      printf("Time t=%.2f        NODE 0:  Sending cost update to Node %d:",get_clock(),k);
...
void rtupdate0 (struct rtpkt *rcvdpkt)
{
   ...
   printf("Time t=%.2f        NODE 0:  Received cost update from Node %d:",get_clock(),sourceid);
   ...
   // 向每个邻居节点发送更新数据包
   if(change == 1){
      printf("Time t=%.2f        NODE 0:  New minimum costs.\n");
      int k,t;
      for(k = 0; k < NODES; k++){
         if(k == id || linkcost[id][k] == INF) continue;
         struct rtpkt packet;
   ...
   else printf("Time t=%.2f        NODE 0:  No change in minimum costs.\n\n");
}
...
void printdt_0 (struct distance_table *t)
{
   ...
   printf("\t\t   D0 |    1    3\n");
   printf("\t\t  ----|-----------------\n");
   printf("\t\t     1|  %3d   %3d\n", t->costs[1][1], t->costs[1][3]);
   printf("\t\tdest 2|  %3d   %3d\n", t->costs[2][1], t->costs[2][3]);
   printf("\t\t     3|  %3d   %3d\n", t->costs[3][1], t->costs[3][3]);
   printf("\t\t     4|  %3d   %3d\n", t->costs[4][1], t->costs[4][3]);
   printf("\t\t  ----------------------\n");
}
